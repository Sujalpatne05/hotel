...
export default function SuperAdminRevenue() {
  ...
  const formatINR = (value: number) => value.toLocaleString("en-IN", { style: "currency", currency: "INR", maximumFractionDigits: 0 });
  const totalRevenueFormatted = formatINR(totalRevenue);
  const highestMonthFormatted = formatINR(32000);
  const lowestMonthFormatted = formatINR(12000);
  return (
    <SuperAdminLayout>
      <div className="flex-1 p-2 sm:p-6 md:p-10">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 sm:gap-8 mb-8">
          <div className="bg-white rounded-xl shadow p-6 flex flex-col items-center border">
            ...
            <div className="text-3xl font-extrabold text-gray-900 mt-2">{formatINR(totalRevenue)}</div>
          </div>
          <div className="bg-white rounded-xl shadow p-6 flex flex-col items-center border">
            ...
            <div className="text-3xl font-extrabold text-gray-900 mt-2">{formatINR(thisMonth)}</div>
          </div>
          <div className="bg-white rounded-xl shadow p-6 flex flex-col items-center border">
            ...
            <div className="text-3xl font-extrabold text-gray-900 mt-2">{formatINR(expenses)}</div>
          </div>
          <div className="bg-white rounded-xl shadow p-6 flex flex-col items-center border">
